...
WACC_HTML_URL = "https://pages.stern.nyu.edu/~adamodar/New_Home_Page/datafile/wacc.html"

# Try to fetch and parse Damodaran's latest WACC table
try:
    res = requests.get(WACC_HTML_URL, timeout=10)
    res.raise_for_status()
    tables = pd.read_html(res.text)             # parse all tables in the HTML
    df = tables[0]                              # first table is the WACC data
    df = df[~df["Industry Name"].str.contains("Total Market")]  # remove aggregate rows
except Exception as e:
    # Fallback: use hardcoded WACC table (Jan 2025 data)
